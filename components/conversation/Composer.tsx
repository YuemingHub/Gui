...
export function Composer({ disabled, restoreText, onSend }: ComposerProps) {
  const [value, setValue] = useState("");
  const [appliedRestore, setAppliedRestore] = useState<string | null>(null);
  const inputRef = useRef<HTMLTextAreaElement | null>(null);

  if (restoreText && restoreText !== appliedRestore) {
    setAppliedRestore(restoreText);
