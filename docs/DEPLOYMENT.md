...
✅ **构建期网络依赖已移除（2026-09-07）**：字体由 `geist` npm 包随依赖提供（`npm ci` 走镜像即可），`npm run build` 不再访问 Google Fonts——清空字体缓存的全新构建已验证通过。`app/lib/fontSelfHost.test.ts` 永久禁止 `next/font/google` 回流。
